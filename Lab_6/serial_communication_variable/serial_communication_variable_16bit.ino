...
boolean sendPacket(unsigned int payloadSize, byte *payload)
{
    ...
    byte checksum[2];
    // $$$ Add the Code Here
    // Put XOR of packet[0] and packet[2] into checksum[0]
    // Put XOR of packet[1] and packet[3] into checksum[1]
    ...
    // store the checksum
    // $$$ Add the Code Here
    packet[packetSize - 2] = // First byte of checksum
    packet[packetSize - 1] = // Second byte of checksum
...
boolean validatePacket(unsigned int packetSize, byte *packet)
{
    ...
    {
        // $$$ Add the Code Here (Hint: Take a look at line 103)
        /*_*/ = /*_*/ ^ packet[i];
    }

    // check to see if the computed checksum and packet checksum are equal
    // $$$ Add the Code Here
    if((packet[packetSize - 2] != checksum[0]) || (/*_*/))
    {
        return false;
    }
