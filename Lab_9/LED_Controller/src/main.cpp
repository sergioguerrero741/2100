...
extern "C" void button_send_clicked(GtkWidget *p_wdgt, gpointer p_data )
{
  ...
  const char * rVal= gtk_entry_get_text(GTK_ENTRY(gui_app->red_in));
  // $$$ Add code here
  // Do the same for green color
  // const char * gVal = ...
  // Do the same for blue color
  ...
  gtk_range_set_value(GTK_RANGE(gui_app->scale_red), r_int);
  // $$$ Add code here
  // Do the same for green color
  // Do the same for blue color

  double red_value = gtk_range_get_value(GTK_RANGE(gui_app->scale_red));
  // $$$ Add code here
  // Do the same for green color
  // Do the same for blue color
  ...
  char buff[7];
  int bufferLength = 7;
  buff[0] = 0xAA; // input AA
  buff[1] = 0x07; //input 7
  buff[2] = 0x4C; //input 4C
  buff[3] = (unsigned char) red_value; //red
  // $$$ Add code here
  // Do the same for green color
  // Do the same for blue color
  buff[6] = buff[0]^buff[1]^buff[2]^buff[3]^buff[4]^buff[5]; //Xor
  ...
  write(ser_dev, buff, bufferLength);
}
...
extern "C" void scale_rgb_value_changed(GtkWidget *p_wdgt, gpointer p_data )
{
  ...
  char c_cc_value[500];
  sprintf(c_cc_value,"%d", (int)red_value);
  gtk_entry_set_text(GTK_ENTRY(gui_app->red_in), c_cc_value);
  // $$$ Add code here
  // Do the same for green color
  //
  // Do the same for blue color
  ...
  buff[1] = 0x07;
  buff[2] = 0x4C;
  buff[3] = (unsigned char) red_value;
  // $$$ Add code here
  // Do the same for green color
  // Do the same for blue color
  buff[6] = buff[0]^buff[1]^buff[2]^buff[3]^buff[4]^buff[5]; //Xor
