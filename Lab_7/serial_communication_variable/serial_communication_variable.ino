        ...
        if(Serial.available())
        {
            ...
            if(count >= packetSize)
            {
                ...
                if(validatePacket(packetSize, buffer))
                {
                    ...
                    // Part I
                    if (buffer[2] == 0x4C) {
                        analogWrite(23,255-buffer[3]);
                        // $$$ Add code here
                        // Do the same for color green (port 22);
                        // Do the same for color red (port 21);
                    }
                    // Part II
                    if (buffer[2] == 0x50) {
                        int x = analogRead(14)/4;
                        byte packetx[2];
                        // $$$ Add code here
                        // put 0x50 in first byte of packetx
                        // put the pot_value in second byte
                        sendPacket(2,packetx);
                    }
                }
                count = 0;
            }
